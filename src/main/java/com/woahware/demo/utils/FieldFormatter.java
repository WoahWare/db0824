...
public class FieldFormatter {
    ...
    public static String convertFieldName(String fieldName) {
        if (fieldName.isEmpty()) {      // if no fieldName, then return null
            return null;
        }
        StringBuilder result = new StringBuilder(fieldName.length());
        result.append(Character.toUpperCase(fieldName.charAt(0)));
        // loop through string, detect next Uppercase letter, and insert space + lowercase version of that letter
        for (int i = 1; i < fieldName.length(); i++) {
            char ch = fieldName.charAt(i);
            if(result.length() < 4 && result.toString().equals("Pre")) {    // edge-case where fields with Pre, need a hyphen (-)
                result.append("-");
            }
